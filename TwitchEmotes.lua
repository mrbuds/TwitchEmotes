TwitchEmotes = LibStub("AceAddon-3.0"):NewAddon("Twitch Emotes")
local TwitchEmotes = TwitchEmotes
...
local dropdown_options = {
    [01] = { --45
        ...
        "asmonGASM", "asmonGet", "asmonHide", "asmonL", "asmonLFR", "asmonLove",
    ...
    [05] = { -- 36
        ...
        "Del", "ednasly", "endANELE","endBomb", "AMAZIN", "pokiW", 
        "ThisIsFine"
    },
    [06] = { -- 30
        ...
        "PogCena","OkayChomp", "mastahFloor", "selyihHEY", "SuicideThinking", "BOGGED",
        "Stonks", "DogeKEK", "sosig","sosigW","LOLW", "OMEGALOL", 
        "ZOINKS"
    },
    [07] = { -- 32
        "Custom 3", "FLOPPERS", "cmoN", "OhISee", "Poghurt", "KEKW",
        "KEKWeird", "KEKSad", "KEKL", "BroKiss", "pOg", "Pogey", 
        ...
        "4Shrug", "4Mansion", "5Head"
    },
    [08] = { -- 29
        "Custom 4", "PogWarts", "noxSorry", "AMAZINGA", "BaconEffect",":fluff:", 
        "PATHETIC", "TriHardS", "HYPERDANSGAMEW", "MaN", "NaM", "VaN", 
        "WhiteKnight", "BlackKnight", "PikaWOW", "Boomer", "Zoomer","Coomer", 
        "WeirdChamp", "WutChamp", "StareChamp", "OkayChamp","MaldChamp", "starechamp", 
    ...
    [13] = { -- 45
        ...
        ":writing_hand:", ":book:", ":clock:", ":bear:", ":crab:", ":tomato:", 
        ":shrimp:", ":lizard:", ":rage:", ":eye:", ":eyes:", ":b:",
    ...
    [22] = { -- 31
        ...
        "Okayge", "Sadge", "PepeFrench", "PepeJew", "monkaGasp", "GiggleHands",
        "monkaSusp", "PepeWizard", "PepeBruh", "PepeGang", "FeelsRottenMan",":need:", 
        ":memes:", ":drama:"
    },
    [23] = { -- 33
        "Pepe 4", "PepePoo", "SadPepe", "PepeXD", "oldpepe", "monakHmm",
        "FeelsOakyMan", "FeelsWiredMan", "PeepHand", "monakS",":think:",":hmm:",
        ...
        "PepeDisgust", "PepeSmug", "FatDank", "PepePains"
    },
    [24] = { -- 33
    ...
    [25] = { -- 33
        ...
        "peepoFriends","peepoSadJAM","peepoCat","peepoSleepo"
    },
    [26] = { -- 33
        "Suze4Mumes Life P1", "beamBrah", "beamB", "CaptainSuze", "peep420","JanCarlo2", 
        ...
        "PepeBed", "PepeChill", "PepeCool", "EZGiggle"
    },
    [27] = { -- 32
        "Suze4Mumes Life P2", "PepeLaffe", "coffeeS", "peepoEZSip", "PepeGiggle", "PepeKingLove", 
...
local function createconfig()
	local function get(info)
        ...
        if ns == "FAVEMOTES" then
            opt = tonumber(opt)
        end
        local val
        if opt then
            val = TwitchEmotes.db.global[ns][opt]
        else
            val = TwitchEmotes.db.global[ns]
        end
	...
	local function set(info, arg1, arg2, arg3, arg4)
        ...
        if opt then
            TwitchEmotes.db.global[ns][opt] = arg1
        else
            TwitchEmotes.db.global[ns] = arg1
        end

		if ns == "channels" then
			TwitchEmotes:SetChannel(opt, arg1)
        elseif ns == "minimap" then
            TwitchEmotes:SetMinimapButton()
		end
    end
	...
	local options = {
        ...
        args = {
            ...
            help = {
                type = "description",
                name = "Select channels in which you want to see the Emoticons . Note: General, Trade, Localdefence etc all come under the category of 'Channels'.",
                order = 3
            },
            channels = {
                type = "group",
                name = "Channels",
                desc = "Select channels in which you want to see the Emoticons . Note: General, Trade, Localdefence etc all come under the category of 'Channels'.",
                inline = true,
                order = 4,
                args = {
                    say = {
                    ...
                    bnetwhisper = {
                        ...
                        set = set,
                        get = get,
                        arg = "channels.CHAT_MSG_BN_WHISPER",
                        order = 9
                    },
                    bnetconversation = {
                        type = "toggle",
                        name = "BNet Conversation",
                        set = set,
                        get = get,
                        arg = "channels.CHAT_MSG_BN_CONVERSATION",
                        order = 10
                    },
                    mail = {
                        type = "toggle",
    ...
    for i, tab in ipairs(dropdown_options) do
        local name = tab[1]
        options.args.favorites.args[name] = {
            type = "toggle",
            name = name,
            order = i,
            set = set,
...
TwitchEmotes.defaultpack = {
    ...
    [":AP:"] = "Interface\\AddOns\\TwitchEmotes\\Custom\\AP.tga:28:28",
    ...
    ["peepoSadJAM"] = "Interface\\AddOns\\TwitchEmotes\\Pepes\\peepoSadJAM.tga:28:28",
...
TwitchEmotes.emoticons = {
    ...
    ["peepoSadJAM"] = "peepoSadJAM",
    ["peeposadjam"] = "peepoSadJAM",
    ["peepoGrove"] = "peepoSadJAM",
...
function ItemTextPageText.SetText(self, msg, ...)
    if TwitchEmotes.db.global.channels.MAIL and msg ~= nil then
        msg = TwitchEmotes.RunReplacement(msg)
    end
...
function TwitchEmotes.ShowDropdown(_, button)
    if button == "RightButton" then
        InterfaceOptionsFrame_OpenToCategory("Twitch Emotes")
    else
        L_ToggleDropDownMenu(1, nil, TwitchEmotes.dropDown, "cursor", 0, 0)
    end
end

function TwitchEmotes:LoadDropdown(level, menuList)
    ...
    if (level or 1) == 1 then
        for k, v in ipairs(dropdown_options) do
            if TwitchEmotes.db.global.FAVEMOTES[k] then
                info.hasArrow = true
                info.text = v[1]
                info.value = false
                info.menuList = k
        ...
        local first = true;
        for _, v in ipairs(dropdown_options[menuList]) do
            if (first) then
                first = false
            else
                info.text = "|T" .. TwitchEmotes.defaultpack[v] .. "|t " .. v
                info.value = v;
                info.func = TwitchEmotes.Dropdown_OnClick
...
function TwitchEmotes:Dropdown_OnClick(arg1, arg2, arg3)
    if (ACTIVE_CHAT_EDIT_BOX ~= nil) then
        ACTIVE_CHAT_EDIT_BOX:Insert(self.value)
    end
end

function TwitchEmotes:MessageFilter(event, msg, ...)
    msg = TwitchEmotes.RunReplacement(msg)

    return false, msg, ...
end

function TwitchEmotes:UpdateChatFilters()
    for k, v in pairs(TwitchEmotes.db.global.channels) do
        if k ~= "MAIL" then
            if (v) then
                ChatFrame_AddMessageEventFilter(k, self.MessageFilter)
            else
                ChatFrame_RemoveMessageEventFilter(k, self.MessageFilter)
            end
        end
    end
end

function TwitchEmotes.RunReplacement(msg)
    ...
    while (startpos <= origlen) do
        ...
        outstr = outstr ..
            TwitchEmotes.InsertEmoticons(string.sub(msg, startpos, endpos)) -- run replacement on this bit
        startpos = endpos + 1
        if (pos ~= nil) then
...
function TwitchEmotes:SetMinimapButton()
    if self.db.global.minimap.show then
...
function TwitchEmotes.InsertEmoticons(msg)
    ...
    for k, v in pairs(TwitchEmotes.emoticons) do
        if (string.find(msg, k, 1, true)) then
            ...
            if not TwitchEmotes.db.global.LARGEEMOTES then
                if ( size == 'LARGE' or size == 'XLARGE' or size == 'XXLARGE' ) then
                    path_and_size = string.gsub(TwitchEmotes.defaultpack[v], size, normal)
                end
            else
                if ( size == 'LARGE' ) then
                    path_and_size = string.gsub(TwitchEmotes.defaultpack[v], size, large)
                elseif ( size == 'XLARGE' ) then
                    path_and_size = string.gsub(TwitchEmotes.defaultpack[v], size, xlarge)
                elseif ( size == 'XXLARGE') then
                    path_and_size = string.gsub(TwitchEmotes.defaultpack[v], size, xxlarge)
                end
            end
...
function TwitchEmotes:SetChannel(chattype, state)
    if (chattype == "CHAT_MSG_RAID") then
        self.db.global.channels.CHAT_MSG_RAID_LEADER = state
        self.db.global.channels.CHAT_MSG_RAID_WARNING = state
    end
    if (chattype == "CHAT_MSG_PARTY") then
        self.db.global.channels.CHAT_MSG_PARTY_LEADER = state
        self.db.global.channels.CHAT_MSG_PARTY_GUIDE = state
    end
    if (chattype == "CHAT_MSG_WHISPER") then
        self.db.global.channels.CHAT_MSG_WHISPER_INFORM = state
    end
    if (chattype == "CHAT_MSG_INSTANCE_CHAT") then
        self.db.global.channels.CHAT_MSG_INSTANCE_CHAT_LEADER = state
    end
    if (chattype == "CHAT_MSG_BN_WHISPER") then
        self.db.global.channels.CHAT_MSG_BN_WHISPER_INFORM = state
    end
    self:UpdateChatFilters()
end

function TwitchEmotes:RegisterPack(name, newEmoticons, pack)
    for k, v in pairs(newEmoticons) do
        self.emoticons[k] = v
    end

    for k, v in pairs(pack) do
        self.defaultpack[k] = v
    end
end

function TwitchEmotes:OnInitialize()
    self.db = LibStub("AceDB-3.0"):New("TwitchEmoteDB", defaults)
    config:RegisterOptionsTable("Twitch Emotes", createconfig(), "Twitch Emotes")
    dialog:AddToBlizOptions("Twitch Emotes")

    self.dropDown = CreateFrame("Frame", "TwitchEmotesDropDown", UIParent, "UIDropDownMenuTemplate")
    L_UIDropDownMenu_Initialize(self.dropDown, self.LoadDropdown, "MENU", 1)

    local Broker_TwitchEmotes = LDB:NewDataObject("TwitchEmotes", {
        type = "launcher",
        text = "TwitchEmotes",
        icon = "Interface\\AddOns\\TwitchEmotes\\1337.tga",
        OnClick = TwitchEmotes.ShowDropdown
    })
    LDBIcon:Register("TwitchEmotes", Broker_TwitchEmotes, TwitchEmotes.db.global.minimap)
